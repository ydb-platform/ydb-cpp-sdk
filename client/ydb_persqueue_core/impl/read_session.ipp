...
//     if constexpr (UseMigrationProtocol) {
//         return Settings.RangesMode_.or_else(RangesMode);
//     } else {
//         return true;
//     }
// }
...
bool TSingleClusterReadSessionImpl<UseMigrationProtocol>::GetRangesMode() const {
    if constexpr (UseMigrationProtocol) {
        if (Settings.RangesMode_.has_value()) {
            return *Settings.RangesMode_;
        } else {
            return RangesMode;
        }
    } else {
            ...
            try {
                ...
                if constexpr (UseMigrationProtocol) {
                    if (Parent->DoDecompress
                        ...
                        std::string decompressed = NCompressionDetails::Decompress(data);
                    ...
                    if (Parent->DoDecompress
                        ...
                        std::string decompressed = ::NYdb::NTopic::NCompressionDetails::Decompress(data, static_cast<Ydb::Topic::Codec>(batch.codec()));
                        data.set_data(decompressed);
                    }
                }
