...
#include <client/impl/ydb_internal/table_helpers/helpers.h>
#include <client/impl/ydb_internal/make_request/make.h>
#include <client/impl/ydb_internal/retry/retry.h>
#include <client/impl/ydb_internal/retry/retry_async.h>
#include <client/impl/ydb_internal/retry/retry_sync.h>
...
#include <client/ydb_table/impl/client_session.h>
#include <client/ydb_table/impl/data_query.h>
#include <client/ydb_table/impl/request_migrator.h>
#include <client/ydb_table/impl/table_client.h>
...
std::optional<TString> TStorageSettings::GetTabletCommitLog0() const {
...
std::optional<TString> TStorageSettings::GetTabletCommitLog1() const {
...
std::optional<TString> TStorageSettings::GetExternal() const {
...
std::optional<TString> TColumnFamilyDescription::GetData() const {
...
public:
        ...
        : TImpl(desc)
    {
        ...
        for (const auto& bound : Proto_.shard_key_bounds()) {
            std::optional<TKeyBound> fromBound = leftValue.has_value()
        ...
        if (describeSettings.WithKeyShardBoundary_) {
            Ranges_.emplace_back(TKeyRange(
                leftValue.has_value() ? TKeyBound::Inclusive(*leftValue) : std::optional<TKeyBound>(),
                std::optional<TKeyBound>()));
        }
    }
    ...
    const std::optional<TString>& GetTiering() const {
        return Tiering_;
    }
...
std::optional<TString> TTableDescription::GetTiering() const {
...
const std::optional<TString>& TDataQuery::GetText() const {
