...
void TLogTest::TestThreaded() {
    {
        ...
        TLog log(std::unique_ptr(new TThreadedLogBackend(&fb)));
...
void TLogTest::TestThreadedWithOverflow() {
    ...
    TFakeLogBackend fb;
    {
        TLog log(std::unique_ptr(new TThreadedLogBackend(&fb, 2)));
    ...
    {
        ui32 overflows = 0;
        TLog log(std::unique_ptr(new TThreadedLogBackend(&fb, 2, [&overflows] { ++overflows; })));
...
void TLogTest::TestFormat() {
    ...
    {
        TLog log(std::unique_ptr(new TStreamLogBackend(&data)));
