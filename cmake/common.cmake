...
function(target_sources_custom TgtName CompileOutSuffix)
  ...
  foreach(Src ${TARGET_SOURCES_CUSTOM_SRCS})
    ...
    _ydb_sdk_apply_source_compile_opts(${SrcCopy}
      "${TARGET_SOURCES_CUSTOM_CUSTOM_FLAGS};-I${YDB_SDK_SOURCE_DIR}/${SrcDir}")
  endforeach()
...
function(_ydb_sdk_apply_source_compile_opts src opts)
  if (NOT opts)
    return()
  endif()
  if (CMAKE_VERSION VERSION_GREATER_EQUAL 3.29)
    set_source_files_properties(${src} PROPERTIES COMPILE_OPTIONS "${opts}")
  else()
    string(REPLACE ";" " " _opts_str "${opts}")
    get_source_file_property(_existing_flags ${src} COMPILE_FLAGS)
    if (_existing_flags STREQUAL "NOTFOUND")
      set(_existing_flags "")
    endif()
    if (_existing_flags)
      set_source_files_properties(${src} PROPERTIES COMPILE_FLAGS "${_existing_flags} ${_opts_str}")
    else()
      set_source_files_properties(${src} PROPERTIES COMPILE_FLAGS "${_opts_str}")
    endif()
  endif()
endfunction()

function(_ydb_sdk_expand_collect_item Item PackageName OutList)
  set(_expanded "")
  if (Item MATCHES "^\\$<BUILD_INTERFACE:([^>]+)>$")
    list(APPEND _expanded ${CMAKE_MATCH_1})
  elseif (Item MATCHES "^\\$<INSTALL_INTERFACE:")
  elseif (Item MATCHES "^\\$<TARGET_FILE:([^>]+)>$")
    if (TARGET ${CMAKE_MATCH_1})
      _ydb_sdk_collect_package_target(${PackageName} ${CMAKE_MATCH_1})
    else()
      message(WARNING "YDB SDK: TARGET_FILE refers to unknown target '${CMAKE_MATCH_1}'")
    endif()
  elseif (Item MATCHES "^\\$<")
    message(WARNING "YDB SDK: unsupported generator expression in package collection: '${Item}'")
  elseif (TARGET ${Item})
    _ydb_sdk_collect_package_target(${PackageName} ${Item})
  else()
    list(APPEND _expanded ${Item})
  endif()
  set(${OutList} "${_expanded}" PARENT_SCOPE)
endfunction()
    ...
    if (tgt_srcs)
      ...
      if (resolved_srcs)
        ...
        foreach(resolved_src IN LISTS resolved_srcs)
          ...
          if (tgt_compile_opts)
            _ydb_sdk_apply_source_compile_opts(${resolved_src} "${tgt_compile_opts}")
          endif()
  ...
  foreach(prop IN ITEMS INCLUDE_DIRECTORIES INTERFACE_INCLUDE_DIRECTORIES)
    ...
    if (target_include_dirs)
      foreach(item IN LISTS target_include_dirs)
        _ydb_sdk_expand_collect_item("${item}" ${PackageName} _collect_expanded)
        if (_collect_expanded)
          set_property(GLOBAL APPEND PROPERTY YDB_CPP_${PackageName}_INCLUDE_DIRS ${_collect_expanded})
        endif()
      endforeach()
    endif()
  endforeach()
  ...
  foreach(prop IN ITEMS LINK_LIBRARIES INTERFACE_LINK_LIBRARIES)
    ...
    if (target_deps)
      foreach(dep IN LISTS target_deps)
        _ydb_sdk_expand_collect_item("${dep}" ${PackageName} _collect_expanded)
        if (_collect_expanded)
          set_property(GLOBAL APPEND PROPERTY YDB_CPP_${PackageName}_PUBLIC_DEPS ${_collect_expanded})
        endif()
      endforeach()
    endif()
  endforeach()
