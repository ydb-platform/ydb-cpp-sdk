...
TMemoryInput::TMemoryInput(const std::string_view buf) noexcept
    : Buf_(buf.data())
    , Len_(buf.size())
{
}
...
size_t TMemoryOutput::DoNext(void** ptr) {
    Y_ENSURE(Buf_ < End_, std::string_view("memory output stream exhausted"));
    *ptr = Buf_;
...
void TMemoryOutput::DoWrite(const void* buf, size_t len) {
    char* end = Buf_ + len;
    Y_ENSURE(end <= End_, std::string_view("memory output stream exhausted"));
