...
Y_UNIT_TEST_SUITE(TypeName) {
    ...
    Y_UNIT_TEST(FromArcadiaTypes) {
        ...
        UNIT_ASSERT_VALUES_EQUAL(TypeName<std::string>(), "class TBasicString<char,struct std::char_traits<char> >");
#else
        UNIT_ASSERT_VALUES_EQUAL(TypeName<yexception>(), "yexception");
        UNIT_ASSERT_VALUES_EQUAL(TypeName<std::string>(), "TBasicString<char, std::char_traits<char> >");
#endif
    }
