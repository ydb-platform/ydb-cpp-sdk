...
Y_UNIT_TEST_SUITE(TSimpleTMTest) {
    std::string PrintMarker(const std::string& test, int line) {
        return TStringBuilder() << "test " << test << " at line " << line;
    }

    std::string JoinMarker(const std::string& marker, time_t t) {
        return TStringBuilder() << marker << " (tstamp=" << t << ")";
    }

    std::string PrintMarker(const std::string& test, int line, time_t t) {
        return JoinMarker(PrintMarker(test, line), t);
    }

    void AssertStructTmEqual(const std::string& marker, const struct tm& tmt, const NDatetime::TSimpleTM& tms) {
