...
def _check_convert(std::map[TString, int] x):
    return x
...
class TestMap(unittest.TestCase):
    ...
    def test_constructors_and_assignments(self):
        cdef std::map[TString, int] c1
        c1["one"] = 1
        c1["two"] = 2
        cdef std::map[TString, int] c2 = std::map[TString, int](c1)
    ...
    def test_equality_operator(self):
        cdef std::map[TString, int] base
        base["one"] = 1
        base["two"] = 2

        cdef std::map[TString, int] c1 = std::map[TString, int](base)
        self.assertTrue(c1==base)

        cdef std::map[TString, int] c2
        c2["one"] = 1
        c2["two"] = 2
        ...
        cdef std::map[TString, int] c3 = std::map[TString, int](base)
        c3["one"] = 0
        self.assertTrue(c3 != base)

    def test_insert_erase(self):
        cdef std::map[TString, int] tmp
    ...
    def test_iterators_and_find(self):
        ...
        tmp["1"] = 1
        self.assertTrue(tmp.begin() != tmp.end())
        cdef std::map[TString, int].iterator it = tmp.find("1")
