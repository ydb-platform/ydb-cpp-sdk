...
template <class TAlg>
void ApplyAsymmetricAlg(TJwtTokenSourceParams* params, const std::string& privateKey) {
...
bool IsAsciiEqualUpper(const std::string& jsonParam, const std::string_view& constantInUpperCase) {
...
struct TFixedTokenSourceParamsForParsing {
    ...
    FLUENT_SETTING(std::string, TokenType);
};

// special struct to apply macros
struct TJwtTokenSourceParamsForParsing : public TJwtTokenSourceParams {
    FLUENT_SETTING(std::string, AlgStr);
    FLUENT_SETTING(std::string, PrivateKeyStr);
    FLUENT_SETTING(std::string, TtlStr);
};
...
TOauth2TokenExchangeParams ReadOauth2ConfigJson(const std::string& configJson, const std::string& tokenEndpoint) {
    try {
