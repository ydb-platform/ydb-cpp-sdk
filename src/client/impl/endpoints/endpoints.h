...
struct TEndpointKey {
    ...
    TEndpointKey(std::string endpoint, std::uint64_t nodeId)
        : Endpoint(std::move(endpoint))
        , NodeId(nodeId)
    {}

    TEndpointKey(std::uint64_t nodeId)
        : Endpoint()
        , NodeId(nodeId)
    {}
    ...
    const std::uint64_t& GetNodeId() const {
        return NodeId;
    }
...
public:
    ...
    using THandleCb = std::function<void(std::uint64_t nodeId, const IObjRegistryHandle& handle)>;
    void ForEachEndpoint(const THandleCb& cb, std::int32_t minPriority, std::int32_t maxPriority, const void* tag) const;
