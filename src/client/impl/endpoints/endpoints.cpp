...
public:
    TObjRegistry(const std::uint64_t& nodeId)
        : NodeId_(nodeId)
    {}
...
static std::int32_t GetBestK(const std::vector<TEndpointRecord>& records) {
    ...
    const std::int32_t bestPriority = records[0].Priority;
...
void TEndpointElectorSafe::ForEachEndpoint(const THandleCb& cb, std::int32_t minPriority, std::int32_t maxPriority, const void* tag) const {
    std::shared_lock guard(Mutex_);

    auto it = std::lower_bound(Records_.begin(), Records_.end(), minPriority, [](const TEndpointRecord& l, std::int32_t r) {
        return l.Priority < r;
    });
