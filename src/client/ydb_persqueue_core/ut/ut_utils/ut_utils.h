...
#include <src/client/ydb_persqueue_core/ut/ut_utils/test_utils.h>
#include <src/client/ydb_persqueue_core/ut/ut_utils/sdk_test_setup.h>
                ...
                while (waitEventFuture.HasValue() && !closed) {
                    ...
                    std::visit(TOverloaded {
                            ...
                            [&](const TSessionClosedEvent& event) {
                                Log << TLOG_INFO << "Got close event: " << event.DebugString() << std::endl;
                                if (!MayStop) {
        ...
        , Thread([idsQueue, this]() {
            while(!Stop) {
                ...
                while (TasksQueue.Dequeue(&f)) {
                    ++CurrentTaskId;
                    std::cerr << "Enqueue task with id " << CurrentTaskId << std::endl;
                    Tasks[CurrentTaskId] = f;
                }
                ui64 id = 0;
                while (ExecIdsQueue->Dequeue(&id)) {
                    ExecIds.push(id);
                    std::cerr << "Got ok to execute task with id " << id << std::endl;
                ...
                while (!ExecIds.empty()) {
                    ...
                    if (iter == Tasks.end())
                        break;
                    std::cerr << "Executing compression of " << id << std::endl;
                    ExecIds.pop();
                    try {
                        (iter->second)();
                    } catch (...) {
                        std::cerr << "Failed on compression call: " << CurrentExceptionMessage() << std::endl;
                        Y_ABORT();
                    }
