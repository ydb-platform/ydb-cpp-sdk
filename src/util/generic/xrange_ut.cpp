...
Y_UNIT_TEST_SUITE(XRange) {
    ...
    void TestSteppedXRangeImpl(int begin, int end, int step, const std::vector<int>& expected) {
        size_t expInd = 0;
    ...
    Y_UNIT_TEST(PointersWorks) {
        std::vector<size_t> data = {3, 1, 4, 1, 5, 9, 2, 6};
    ...
    class TVectorChild: public std::vector<size_t> {
    public:
    ...
    Y_UNIT_TEST(ConvertionWorks) {
        ...
        std::vector<size_t> convertionResults[] = {xrange<size_t>(9),
                                               xrange<ui32>(0, 9),
                                               xrange(0, 9, 1)};
    ...
    void TestEmptyRanges(const XRangeContainer& c) {
        for (const auto& emptyRange : c) {
            ...
            const std::vector<TValueType> asVector = emptyRange;
    ...
    static void TestIteratorDifferenceImpl(TRange range, int a, int b, std::optional<int> step) {
        auto fmtCase = [&]() -> std::string { return TStringBuilder() << "xrange(" << a << ", " << b << (step ? ", " + ToString(*step) : std::string{}) << ")"; };
