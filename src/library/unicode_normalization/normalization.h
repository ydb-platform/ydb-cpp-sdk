...
namespace NUnicode {
    ...
    namespace NPrivate {
        ...
        typedef std::vector<TSymbol> TBuffer;
    ...
    public:
        ...
        inline void Normalize(const TCharType* begin, const TCharType* end, std::u32string& out) {
            if (NPrivate::Normalized<Norm>(begin, end)) {
                for (const TCharType* i = begin; i != end;) {
        ...
        template <typename TCharType>
        inline std::basic_string<TCharType> Normalize(const std::basic_string<TCharType>& src) {
            if (NPrivate::Normalized<Norm>(src.data(), src.data() + src.size())) {
                // nothing to normalize
                return src;
            } else {
                std::basic_string<TCharType> res;
                res.reserve(src.length());
                DoNormalize(src.data(), src.data() + src.size(), res);
                return res;
            }
        }
    };
...
inline std::basic_string<TCharType> Normalize(const TCharType* str, size_t len) {
    std::basic_string<TCharType> res;
    res.reserve(len);
...
template <NUnicode::ENormalization N, typename TCharType>
inline std::basic_string<TCharType> Normalize(const std::basic_string<TCharType>& str) {
    ::NUnicode::TNormalizer<N> dec;
    return dec.Normalize(str);
}

template <NUnicode::ENormalization N, typename TCharType>
inline std::basic_string_view<TCharType> Normalize(const std::basic_string_view<TCharType> str) {
