...
namespace NDbgDump {
    namespace NColorScheme {
        ...
        public:
            ...
            // Background color modifiers
            // TODO: support backgrounds in src/library/colorizer
            DBG_OUTPUT_COLOR_HANDLER(Key) {
                if (Depth++ == 0 && Colors.IsTTY()) {
                    stream << DumpRaw(std::string_view("\033[42m"));
                }
            }
            DBG_OUTPUT_COLOR_HANDLER(Value) {
                if (Depth++ == 0 && Colors.IsTTY()) {
                    stream << DumpRaw(std::string_view("\033[44m"));
                }
            }
            DBG_OUTPUT_COLOR_HANDLER(ResetRole) {
                if (--Depth == 0 && Colors.IsTTY()) {
                    stream << DumpRaw(std::string_view("\033[49m"));
                }
            }
