...
Y_UNIT_TEST_SUITE(TRequestServerDataTest) {
    Y_UNIT_TEST(Headers) {
        ...
        UNIT_ASSERT_VALUES_EQUAL(sd.HeaderInOrEmpty("X-XX"), std::string_view("y-yy"));
        UNIT_ASSERT_VALUES_EQUAL(sd.HeaderInOrEmpty("X-XXX"), std::string_view("y-yyy"));
    }
    ...
    Y_UNIT_TEST(ComplexHeaders) {
        ...
        UNIT_ASSERT_VALUES_EQUAL(sd.HeadersCount(), 2);
        UNIT_ASSERT_VALUES_EQUAL(sd.HeaderInOrEmpty("X-Xz"), std::string_view("y-yy"));
        ...
        UNIT_ASSERT_VALUES_EQUAL(sd.HeaderInOrEmpty("Host"), std::string_view("1234"));
        UNIT_ASSERT_VALUES_EQUAL(sd.ServerName(), "1234");
        sd.AddHeader("Host", "12345:678");
        UNIT_ASSERT_VALUES_EQUAL(sd.HeadersCount(), 3);
        UNIT_ASSERT_VALUES_EQUAL(sd.HeaderInOrEmpty("Host"), std::string_view("12345:678"));
    ...
    Y_UNIT_TEST(Ctor) {
        const std::string qs("gta=fake&haha=da");
    ...
    Y_UNIT_TEST(HashCut) {
        const std::string qs("&gta=fake&haha=da");
        const std::string header = " /yandsearch?" + qs + "#&uberParam=yes&q=? HTTP 1.1 OK";
    ...
    Y_UNIT_TEST(AppendQueryString) {
        ...
        constexpr std::string_view appendix = "gta=true&gta=new";
    ...
    Y_UNIT_TEST(SetRemoteAddrSimple) {
        static const std::string TEST = "abacaba.search.yandex.net";
    ...
    Y_UNIT_TEST(SetRemoteAddrRandom) {
        for (size_t size = 0; size < 2 * INET6_ADDRSTRLEN; ++size) {
            const std::string test = NUnitTest::RandomString(size, size);
            TServerRequestData rd;
            rd.SetRemoteAddr(test);
