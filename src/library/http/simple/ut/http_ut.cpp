...
Y_UNIT_TEST_SUITE(SimpleHttp) {
    ...
    Y_UNIT_TEST(keepaliveWithClosedByPeer) {
        ...
        std::unique_ptr<NMock::TMockServer> server = std::unique_ptr<NMock::TMockServer>(createOptions(port, true), gen);

        TKeepAliveHttpClient cl("localhost", port);
        UNIT_ASSERT_NO_EXCEPTION(cl.DoGet("/ping"));

        server.Reset();
        server = std::unique_ptr<NMock::TMockServer>(createOptions(port, true), gen);
        UNIT_ASSERT_NO_EXCEPTION(cl.DoGet("/ping"));
