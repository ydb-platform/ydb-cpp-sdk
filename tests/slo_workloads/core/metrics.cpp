...
public:
    TOtelMetricsPusher(
        ...
        , CommonAttributes_(resourceAttributes)
        , MeterSchemaVersion_(meterSchemaVersion)
    {
        ...
        Meter_ = MeterProvider_->GetMeter("slo_workloads", MeterSchemaVersion_);
...
private:
    void InitMetrics() {
        ...
        RetryAttempts_ = Meter_->CreateInt64Gauge(
            "sdk_retry_attempts",
    ...
    std::string MeterSchemaVersion_;
...
std::unique_ptr<IMetricsPusher> CreateOtelMetricsPusher(
    ...
    return std::make_unique<TOtelMetricsPusher>(metricsPushUrl, operationType, resourceAttributes, meterSchemaVersion);
}
