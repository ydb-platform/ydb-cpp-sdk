...
int main(int argc, char** argv) {
    ...
    const QString connectionName = QStringLiteral("ydb-odbc-package-test");
    {
        ...
        if (!query.exec(QStringLiteral("SELECT 42 AS value"))) {
            std::cerr << "QODBC query failed: "
                      << query.lastError().text().toStdString() << std::endl;
            return 5;
        }
        if (!query.next()) {
            std::cerr << "QODBC returned no row: "
                      << query.lastError().text().toStdString() << std::endl;
            return 6;
        }
        const QVariant value = query.value(0);
        if (value.toInt() != 42) {
            std::cerr << "QODBC returned an unexpected SELECT result: type="
