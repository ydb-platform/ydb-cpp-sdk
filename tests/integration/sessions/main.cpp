...
void WarmPoolCreateSession(NYdb::NQuery::TQueryClient& client, std::string& sessionId) {
    ...
    auto result = session.ExecuteQuery(R"___(
        SELECT 42;
    )___", NYdb::NQuery::TTxControl::BeginTx().CommitTx()).ExtractValueSync();
    ASSERT_TRUE(result.IsSuccess());
    sessionId = session.GetId();
}

void WaitForSessionsInPool(NYdb::NQuery::TQueryClient& client, std::int64_t expected) {
    while (client.GetActiveSessionCount() != expected) {
...
TEST(YdbSdkSessions, CloseSessionWithSessionPoolExplicit) {
    ...
    while (iterations--) {
        ...
        if (dis(gen) == 5) {
            auto future = client.Stop();
            future.Apply([](NThreading::TFuture<void> f) {
                return f;
            }).Wait();
        } else {
            client.Stop().Wait();
